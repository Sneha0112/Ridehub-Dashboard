...
--checking duplicate trip id present or not 
...
select sum(searches_got_estimate) searches_estimate from trips_details4;

--total searches which got qutoes
select sum(searches_got_quotes) searches_quotes from trips_details4;
...
--total end ride
select sum(end_ride) end_ride from trips_details4;
...
group by faremethod 
order by count(tripid) desc ) b on
a.id=b.faremethod;

--highest payment  was made through which method
select a.method from payment a inner join
(select top 1 * from trips
order by fare desc) b on
a.id=b.faremethod;
...
from
(select loc_from ,loc_to ,count(distinct tripid) total_trips from trips 
...
--which driver ,cutomer pair had more orders
select * from 
(select * ,rank() over(order by cnt desc) rnk from
...
group by duration,loc_from)a)c where rnk=1;
...
from
(select loc_from,count(*) - sum(driver_not_cancelled) can from 
trips_details4 
group by loc_from)b)c
where rnk=1;
...
select * from (select *,rank() over(order by can desc) rnk
from
(select loc_from,count(*) - sum(customer_not_cancelled) can from 
trips_details4 
group by loc_from)b)c
where rnk=1;
